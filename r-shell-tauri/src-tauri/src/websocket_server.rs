...
#[derive(Clone)]
struct ActivePtyStream {
    generation: u64,
    flow_control: PtyFlowControl,
}

type ActivePtyStreams = Arc<Mutex<HashMap<String, ActivePtyStream>>>;
...
impl WebSocketServer {
    ...
    async fn handle_connection(&self, stream: TcpStream) -> Result<()> {
        ...
        // Bound the per-socket queue so PTY output backpressures instead of
        // growing unboundedly if rendering or the socket falls behind.
        let (tx, mut rx) = mpsc::channel::<String>(WS_SEND_QUEUE_CAPACITY);
        let active_streams: ActivePtyStreams = Arc::new(Mutex::new(HashMap::new()));
                ...
                Ok(Message::Text(text)) => {
                    ...
                    let ws_msg: InboundWs = match serde_json::from_str(&text) {
                        ...
                        Err(e) => {
                            ...
                            if tx.send(serde_json::to_string(&error)?).await.is_err() {
                                break;
                            }
                            continue;
                        }
                    ...
                    match self
                        .handle_message(ws_msg, tx.clone(), active_streams.clone())
                        .await
                    {
                        Ok(_) => {}
                        Err(e) => {
        ...
        let active_generations = {
            ...
            streams
                .drain()
                .map(|(connection_id, stream)| (connection_id, stream.generation))
                .collect::<Vec<_>>()
        };
        for (connection_id, generation) in active_generations {
            if let Err(e) = self
                .connection_manager
            ...
            {
                tracing::debug!(
                    "Failed to close PTY during WS cleanup for {}: {}",
                    connection_id,
                    e
                );
            }
    ...
    async fn handle_message(
        ...
        active_streams: ActivePtyStreams,
    ) -> Result<()> {
        match msg {
            InboundWs::StartPty {
                connection_id,
                ...
                let (flow_control, mut pause_rx) = PtyFlowControl::new();
                {
                    let mut streams = active_streams.lock().await;
                    streams.insert(
                        connection_id.clone(),
                ...
                // then drains any already-queued chunks up to 32 KiB.
                        ...
                        if *pause_rx.borrow() {
                            match tokio::select! {
                                ...
                                _ = cancel_token.cancelled() => {
                                    tracing::info!("PTY reader cancelled while paused for {}", connection_id_clone);
                                    break;
                                }
                                changed = pause_rx.changed() => changed,
                            } {
                                Ok(()) => continue,
                                Err(_) => {
                                    tracing::debug!(
                                        "Flow control dropped, stopping PTY reader for {}",
                                        connection_id_clone
                                    );
                                    break;
                                }
            ...
            InboundWs::StartDesktop { connection_id } => {
