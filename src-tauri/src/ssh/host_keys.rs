...
/// Slot that a `Client` instance writes a mismatch error into during the SSH
/// handshake. The caller of `connect` reads it after the error to build a
/// descriptive user-facing message.
pub type MismatchSlot = Arc<std::sync::Mutex<Option<HostKeyMismatch>>>;
...
impl HostKeyStore {
    ...
    pub async fn verify(
        ...
        if guard.is_none() {
            *guard = Some(Self::load_from_disk(&self.path).await.unwrap_or_default());
        }
